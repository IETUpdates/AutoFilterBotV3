...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAACAgUAAxkBAAEMloBhj-qhK-p7ESCRHcAwv-YncCugKwACvQMAAmmoeVRaAU63Qh874iIE')
            return
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        btn = []
        search = message.text
        mo_tech_yt = f"**🗂️ Title:** {search}\n**⭐ Rating:** {random.choice(RATING)}\n**🎭 Genre:** {random.choice(GENRES)}\n**📤 Uploaded by [{message.chat.title}](t.me/MM_MOVIESS)**™"
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            try:
                ...
                await query.answer("You are using this for one of my old message, please send the request again.അതായത് ഒന്ന് കൂടി request ചെയ്യ്..",show_alert=True)
                return
            ...
            buttons = [
                [
                    InlineKeyboardButton('⚙️ OWNER', url=f'https://t.me/IET_Owner')
                ]
                ]
